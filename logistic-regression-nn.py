...
# Loading the data (eagle/non-eagle)
...
def model(X_train, Y_train, X_test, Y_test, num_iterations = 2000, learning_rate = 0.5, print_cost = False):
    ...
    print("")
    print("Training model complete. Displaying train and test results:")
...
if (correctIds.shape[0] >= 3 and errorIds.shape[0] >= 3):
    ...
    # Display 3 random correct classifications and 3 random errors in two seperate grids
    im1 = test_set_x[:, correctIds[0]].reshape((num_px, num_px, 3))
    im2 = test_set_x[:, correctIds[1]].reshape((num_px, num_px, 3))
    im3 = test_set_x[:, correctIds[2]].reshape((num_px, num_px, 3))
    im4 = test_set_x[:, errorIds[0]].reshape((num_px, num_px, 3))
    im5 = test_set_x[:, errorIds[1]].reshape((num_px, num_px, 3))
    im6 = test_set_x[:, errorIds[2]].reshape((num_px, num_px, 3))

    fig = plt.figure(figsize=(6., 6.))
    fig.canvas.set_window_title('Correct predictions')
    grid = ImageGrid(fig, 111,  # similar to subplot(111)
                    nrows_ncols=(1, 3),  # creates 3x3 grid of axes
    ...
    fig2.canvas.set_window_title('Incorrect predictions')
    grid2 = ImageGrid(fig2, 111,  # similar to subplot(111)
                    nrows_ncols=(1, 3),  # creates 3x3 grid of axes
                    axes_pad=0.1,  # pad between axes in inch.
                    )

    for ax, im in zip(grid2, [im4, im5, im6]):
        # Iterating over the grid returns the Axes.
        ax.imshow(im)
